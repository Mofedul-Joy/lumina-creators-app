...
// is a clickable thumbnail that PLAYS the real post inline on Lumina (VideoModal)
// instead of bouncing to a dead "Explore more on <platform>" tab. Falls back to a
// play tile when a thumbnail hasn't resolved yet.
...
export function ExampleVideos({ examples, urls }: { examples?: ExampleVideo[]; urls?: string[] }) {
  ...
  const [playing, setPlaying] = useState<ExampleVideo | null>(null);
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {shown.map((v, i) => (
          <button
            ...
            className="group relative flex aspect-[9/16] cursor-pointer items-center justify-center overflow-hidden rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-surface-2)] transition hover:border-[var(--color-brand)]/60"
          >
            {v.thumbnail_url ? (
