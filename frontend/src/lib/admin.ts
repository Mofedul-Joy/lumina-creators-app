...
// watch/approve/decline bar inside the message thread. Returns the FULL
// submission row so the message-thread review reuses the same detail modal (and
// the same full action set) as the Submissions page.
export type PendingReview = AdminSubmission;
export const listCreatorPendingReviews = (creatorId: string) =>
  apiFetch<AdminSubmission[]>(`/api/admin/submissions/by-creator/${creatorId}/pending-review`, auth());
