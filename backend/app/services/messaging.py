...
def list_for_admin(db: Session, *, unread_only: bool = False) -> list[dict]:
    """Every conversation that has at least one message, newest activity first,
    with the creator's identity + this side's unread flag."""
    rows = db.execute(
...
def admin_unread_count(db: Session) -> int:
    convs = db.scalars(select(Conversation).where(Conversation.last_message_at.isnot(None))).all()
